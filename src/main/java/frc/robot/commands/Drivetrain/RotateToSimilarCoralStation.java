...
public class RotateToSimilarCoralStation extends Command {
    ...
    public RotateToSimilarCoralStation(Swerve swerve, DoubleSupplier translationSupplier, DoubleSupplier strafeSupplier) {
        m_Swerve = swerve;
    ...
    @Override
    public void execute() {
        double[] DriverInput = Joysticks.processJoystick(m_TranslationSupplier, m_StrafeSupplier, () -> 0);
    ...
    public void end(boolean interrupted) {
        ...
        if (Math.sqrt(Math.pow(m_Swerve.getChassisSpeeds().vxMetersPerSecond, 2) + Math.pow(m_Swerve.getChassisSpeeds().vyMetersPerSecond, 2)) < .05) {
