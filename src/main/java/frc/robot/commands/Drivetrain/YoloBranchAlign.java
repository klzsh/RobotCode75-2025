...
public class YoloBranchAlign extends Command {
    ...
    // private final PIDController rotationController; // add if needed, poss just pass through a heading or do this as a seperate command
    ...
    public YoloBranchAlign(Swerve swerve, ObjectDetetectorCamera brachDetectorCamera, boolean alignLeft) {
        m_Swerve = swerve;
    ...
    public void execute() {
        ...
        if (!yController.atSetpoint()) {
            if (isLeft) { // use leftmost for target indexing in photonvision
                ...
                currentYaw = m_BranchDetectorCamera.getTargetYaw(m_BranchDetectorCamera.getNumTargets().getAsInt() - 1);
            }
    ...
    public boolean isFinished() {
        ...
        return (m_Swerve.getChassisSpeeds().vxMetersPerSecond < stallSpeedThreshold && desiredSpeeds.vxMetersPerSecond == driveIntoReefSpeed);
    }
